...
const cuisines: CuisineType[] = ['Indian', 'Italian', 'Mexican', 'American', 'Chinese', 'Japanese', 'Thai', 'Mediterranean', 'French', 'Korean', 'Vietnamese', 'Greek', 'Spanish', 'Middle Eastern', 'Cajun', 'German'];
const meals: MealType[] = ['Breakfast', 'Lunch', 'Dinner', 'Snack'];
const proteins: ProteinType[] = ['Ground Beef', 'Chicken', 'Pork', 'Tofu', 'Fish', 'Beans', 'Lentils', 'Beef', 'Shrimp', 'Lamb', 'Turkey', 'Eggs', 'Salmon', 'None'];
...
export default function RecipeGenerator() {
    ...
    const handleGroceryDecision = (addToGrocery: boolean) => {
        if (addToGrocery && generatedRecipe) {
            addToGroceryList(generatedRecipe.id);
        }
        setStep(0);
        setGeneratedRecipe(null);
        setShowGroceryPopup(false);
    ...
    const StepCard = ({ title, options, selected, onSelect, onNext }: { title: string, options: string[], selected: string, onSelect: (val: any) => void, onNext: () => void }) => (
                ...
                <AnimatePresence mode="wait" custom={direction}>
                    ...
                    {step === 3 && (
                        <motion.div
                            ...
                            className="absolute inset-0 overflow-y-auto px-8 py-4"
                        >
                            <h2 className="text-2xl font-bold text-gray-800 mb-8 text-center">Review your choices</h2>
                            ...
                            <div className="flex gap-4 pb-4">
                                ...
                                >
                                    {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : 'Generate Recipe'}
                                </button>
                            </div>
                        </motion.div>
                    )}
                        ...
                        >
                            <div className="flex-1 overflow-y-auto px-8 pb-4">
                                <div className="text-center mb-6 pt-4">
                                    <h2 className="text-2xl font-bold text-gray-800 mb-2">{generatedRecipe.title}</h2>
                                    <p className="text-gray-600 text-sm">{generatedRecipe.description}</p>
                                </div>

                                <div className="flex justify-center gap-6 text-sm text-gray-500 mb-6">
                                    <span className="bg-blue-50 px-3 py-1 rounded-full text-blue-700 font-medium">Prep: {generatedRecipe.prepTime}</span>
                                    <span className="bg-green-50 px-3 py-1 rounded-full text-green-700 font-medium">Cook: {generatedRecipe.cookTime}</span>
                                </div>
                            ...
                            <div className="p-6 bg-white/80 backdrop-blur-md border-t border-gray-100 space-y-3 z-10">
                                {!showGroceryPopup ? (
                                    ...
                                    <div className="p-4 bg-blue-50 border border-blue-200 rounded-xl animate-in fade-in zoom-in-95 duration-200">
                                        <p className="text-blue-800 font-medium mb-3 text-center">Recipe Saved!</p>
                                        <p className="text-sm text-blue-700 mb-4 text-center">Add ingredients to your grocery list?</p>
