    ...
    if (weeklyPlan.length === 0) {
        return (
            <div className="text-center p-12 bg-gray-50 rounded-xl border border-dashed border-gray-300">
                ...
                <p className="mt-1 text-gray-500">Add recipes from your collection to plan your week.</p>
                <Link href="/collection" className="mt-6 inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
                    Browse Collection
                </Link>
            </div>
        );
                ...
                {weeklyPlan.map((recipe) => (
                    <div key={recipe.id} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden flex flex-col">
                         ...
                         <div className="bg-gray-50 px-5 py-3 border-t border-gray-100 flex justify-between items-center">
                            <Link href={`/recipe/${recipe.id}`} className="text-sm font-medium text-blue-600 hover:text-blue-500">
                                View Recipe
                            </Link>
                            <button
                                onClick={() => removeFromWeeklyPlan(recipe.id)}
